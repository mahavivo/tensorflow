...
# 把输入shape转换成了4D tensor，第二与第三维度对应的是照片的宽度与高度，最后一个维度是颜色通道数，本例子中是1
...
# 定义一个tensor来保存shape为[5,5,1,32]权重矩阵W：
...
# 创建第二个卷积层，64个filters，窗口大小为5*5。此时需传递32个channels，因为这是前一层的输出结果
...
# 对12*12排列应用5*5的窗口，步长为1时，卷积层的输出结果是7*7维度
# 下一步对输出的7*7增加一个全连接层，最终结果输入给softmax层
# tensor的第一维度表示第二层卷积层的输出，大小为7*7带有64个filters
# 第二个参数是层中的神经元数量，可自由设置，这里使用包含1024个神经元的一层来处理整个图片
# 权重与bias的tensor如下
W_fc1 = weight_variable([7 * 7 * 64, 1024])
b_fc1 = bias_variable([1024])

# softmax需要将图片打平成一个vector作为输入
# 打平后的vector乘以权重矩阵W_fc1，加上bias b_fc1，最后应用ReLU激活函数
h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 64])
h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

# 使用神经网络中的dropout技术来减少有效参数的数量
